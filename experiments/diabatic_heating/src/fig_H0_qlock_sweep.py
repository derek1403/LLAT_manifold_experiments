...
ΔPV dipole scalars at nominal hour 24 vs heating amplitude: moist (heating with q
free, solid) against its q-locked twin (δq ≡ 0 every step, dashed). Left panel is
...
If PV generation by θ̇ were the dry process it is on paper, locking q would do
nothing and the curves would overlie. They do — but only below ~2 K; above that
the moist curve grows superlinearly while the q-locked one falls back toward
...
def plot(lead_hr: int = 24, style: str = "note"):
    S.apply()
    moist = D.sweep(D.FAMILY["heating_moist"][0], lead_hr)
    qlock = D.sweep(D.FAMILY["heating_qlock"][0], lead_hr)
    ...
    for ax, (key, name) in zip((axL, axR), D.POLES):
        ...
        for init in sorted(set(moist) | set(qlock)):
            ...
            for pts, ls, mfc, tag in ((moist.get(init), "-", c, "moist (q free)"),
                                      (qlock.get(init), "--", "none", "q-locked (δq=0)")):
                if not pts:
                    continue
    ...
    fig.suptitle("H0 — moisture binding of the heating→PV response: moist vs q-locked")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--out", default=str(D.FIGS / "h0_qlock_sweep.png"))
    a = ap.parse_args()

...
pressure fall → inflow. 2×2 grid: free reference, then lock q / w / z.
...
_PANELS = [("heating_moist", "tseries_5K_120h_init{init}", "free heating (reference)"),
           ("heating_qlock", "tseriesq_5K_120h_init{init}", "lock δq (δq≡0)"),
           ("heating_wlock", "tseriesw_5K_120h_init{init}", "lock δw (δw≡0)"),
           ("heating_zlock", "tseriesz_5K_120h_init{init}", "lock δz (δz≡0)")]


def plot(init: str = D.STRONG, style: str = "note"):
    S.apply()
    from llat_manifold.diagnostics.response import loop_series
    series = [loop_series(str(D.run(fam, tag.format(init=init)))) for fam, tag, _ in _PANELS]
...
if __name__ == "__main__":
    ...
    ap.add_argument("--out", default=str(D.FIGS / "m4_loop_sequence.png"))
    a = ap.parse_args()
